...
from research_claw import __version__

console = Console()
app = typer.Typer(
    name="rclaw",
    help="🦀 ResearchClaw — Research paper analysis pipeline with citation crawling and knowledge graphs",
...
def version_callback(value: bool):
    if value:
        console.print(f"[bold cyan]ResearchClaw[/] v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """🦀 ResearchClaw — Research paper analysis pipeline."""
    pass
...
def analyze(
    ...
    console.print(Panel.fit(
        "[bold cyan]🦀 ResearchClaw[/bold cyan]\n"
...
def build(
    ...
    _setup_logging(verbose)
    from research_claw.config import RClawConfig
    from research_claw.pipeline import run_build
...
def export(
    ...
    _setup_logging(verbose)
    from research_claw.config import RClawConfig
    from research_claw.pipeline import run_export
...
def search(
    ...
    _setup_logging(verbose)
    from research_claw.pipeline import run_search
...
def dimensions(
    ...
    _setup_logging(verbose)
    from research_claw.analyze.dimensions import list_available_dimensions
...
def init(
    ...
    """Initialize a ResearchClaw project (creates rclaw.yaml and .env)."""
    _setup_logging(verbose)
    ...
    if env_path.exists():
        ...
        env_path.write_text(
            "# ResearchClaw API Keys\n"
            "RCLAW_API_KEY=sk-...\n"
            "# RCLAW_SCHOLAR_API_KEY=\n",
            encoding="utf-8",
        )
...
def info(
    ...
    console.print(Panel.fit(
        f"[bold cyan]ResearchClaw[/] v{__version__}\n\n"
        f"Model: {config.default_model}\n"
